...
public class AddressController {
    ...
    public Result<List<Address>>  findAddressList(){
        Map<String, String> userInfo = TokenDecode.getUserInfo();
        String username = userInfo.get("username");
        List<Address> address = addressService.getAddress(username);
        return new Result<>(true,StatusCode.OK,"获取用户地址信息成功",address);
    }
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Integer")
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('admin','user')")//限制管理员和user才可以访问
    public Result<Address> findById(@PathVariable(value = "id") Integer id){
    ...
    @GetMapping
    @PreAuthorize("hasAnyAuthority('admin')")//限制管理员才可以访问
